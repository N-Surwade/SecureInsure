<?php
// buy_health.php - Health policy purchase form
session_start();
...
if (isset($_GET['policy'])) {
    ...
    $stmt = $conn->prepare("SELECT policy_name, premium FROM policies WHERE id = ? AND type = 'Health'");
...
<body>
    ...
    <main class="container">
        <h1>Health Insurance - <?php echo htmlspecialchars($policy_name); ?></h1>
        <div class="card">
        ...
        <?php else: ?>
            ...
            <?php endif; ?>
           <form id="healthForm" class="flex-form" action="process_buy.php" method="POST">
                ...
                <input type="hidden" name="policy_type" value="Health">
                <input type="hidden" name="return_url" value="buy_health.php?policy=<?php echo isset($_GET['policy']) ? intval($_GET['policy']) : ''; ?>">
    ...
    <input type="checkbox" id="pre_existing" name="pre_existing" value="yes"
           style="width:18px; height:18px; cursor:pointer;">
    
    <label for="pre_existing" style="margin:0; cursor:pointer;">
        Have pre-existing conditions?
    </label>
</div>
                <button type="submit" class="btn btn-primary">Purchase Health Policy</button>
            </form>
        <?php endif; ?>
    </main>
    <footer>
    ...
    <script id="9v2lq3">
document.getElementById("pre_existing").addEventListener("change", function() {
